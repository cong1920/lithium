/**
 * bench_drt.cc â€?Microbenchmark for dynamic_routing_table
 *
 * Measures:
 *   1. Route registration (insert) throughput
 *   2. Route lookup (find) throughput â€?both hits and misses
...
static std::vector<std::string> generate_routes() {
  // Mix of static, parameterized, and deep paths â€?realistic REST API surface
...
int main(int argc, char** argv) {
  ...
  std::printf("  Lookup URLs (hits) : %zu Ã— %d = %ld\n", lookup_urls.size(),
              lookup_multiplier, (long)lookup_urls.size() * lookup_multiplier);
  std::printf("  Lookup URLs (miss) : %zu Ã— %d = %ld\n", miss_urls.size(),
